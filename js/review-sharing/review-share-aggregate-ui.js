...
export function renderSharedReviewsUI(els, vm, onTimeSeekClick) {
  ...
  if (els.sharedReviewersList) {
    ...
    vm.reviewers.forEach(r => {
      ...
      if (r.isLocal) {
        badge.textContent = t('share.reviewerSelfBadge');
  ...
  if (els.sharedTagsList) {
    ...
    if (vm.tags.length === 0) {
      ...
      span.textContent = t('share.noTagsAssigned');
      els.sharedTagsList.appendChild(span);
    } else {
      vm.tags.forEach(tItem => {
        const chip = document.createElement('span');
        chip.className = 'tag-chip';
        chip.style.cursor = 'default';

        const label = document.createElement('span');
        label.textContent = tItem.tag;
        ...
        countSpan.textContent = `x${tItem.sources.length}`;
        chip.appendChild(countSpan);

        // Tooltip displaying source reviewers
        const sourceNames = tItem.sources.map(s => s.reviewerName).join(', ');
        chip.title = t('share.tooltipAssignedBy', { names: sourceNames });
  ...
  if (els.sharedTimelineList) {
    ...
    if (vm.timelineComments.length === 0) {
      ...
      p.textContent = t('share.noTimelineComments');
      ...
      vm.timelineComments.forEach(c => {
        ...
        if (c.isLocal) {
          ...
          selfBadge.textContent = t('share.reviewerSelfBadge');
